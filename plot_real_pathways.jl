...
for j in 0:ITERATE_TO
    ...
    for i in 1:members
        ...
        for t = 1:size(h_data, 3)
            ice_mass[t] = (sum(h_data[:, :, t])-sum(s_data[:, :, t]))  * 2.0e3 * 2.0e3 / 1e12
        end
...
# save plot
savefig(p, "plots/true_ice_mass_vs_time.png")  # Save the plot to a file
println("Plot saved as 'true_ice_mass_vs_time.png'")
